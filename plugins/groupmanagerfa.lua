...
if matches[1] == "تنظیم نام" and matches[2] then
...
local id = msg.to.id
   ...
   tdcli.migrateGroupChatToChannelChat(id)
    return edit_msg(msg.to.id, msg.id, '_Group Has Been Changed To SuperGroup!_', "md")
     ...
     if msg.to.type == "channel" then
    if matches[1] == "بی صدا" then
    if matches[2] == "همه" then
                    local hash = 'mute_gp:'..chat
                    if redis:get(hash) then
                    return edit_msg(msg.to.id, msg.id, "بی صدا همه فعال شد", "md")
                else
                    redis:set(hash, true)
                    return edit_msg(msg.to.id, msg.id, "بی صدا همه فعال شد", "md")
                     end
                  end
    elseif matches[1] == "با صدا" then
      if matches[2] == 'همه' then
                    local hash = 'mute_gp:'..chat
                    if not redis:get(hash) then
                    return edit_msg(msg.to.id, msg.id, "بی صدا همه فعال نیست", "md")
                else
                    redis:del(hash)
                    return edit_msg(msg.to.id, msg.id, "بی صدا همه غیر فعال شد", "md")
                   end
					   end
             end
